---
name: workspace-demo-hooks-tour
...
This skill **creates one temporary hook file** at `.github/hooks/workspace.demo.hooks-tour.json`
...
> 📌 If you stop or cancel this skill early, run
> `powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .github/scripts/workspace/demo/install-tour-hook.ps1 -Remove` (the cleanup step) to delete the hook and the `demo-hooks-tour`
...
1. **Arm the tour and install the one hook that covers every event.** Run the install script. It
   ...
   ```powershell
   powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .github/scripts/workspace/demo/install-tour-hook.ps1
   ```

   The script prints `INSTALLED=1`, the `HOOK=` path it wrote, and the `LOG=` path where events will
   be recorded. The hook JSON lives **in the script**, not in this skill — one job, one home. Tell
...
3. **PostToolUse — react after a tool runs, and watch a hook chain work on its own.** This event
   ...
   b. **Start a tiny background task.** Kick off ~5 seconds of simulated background work that drops a
      ...
      ```powershell
      powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .github/scripts/workspace/demo/chain-worker.ps1 -DelaySeconds 5
      ```
...
10. **Remove everything.** Run the install script with `-Remove` to delete the temporary hook and
    ...
    ```powershell
    powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .github/scripts/workspace/demo/install-tour-hook.ps1 -Remove
    ```
...
- **Nothing was embedded in this skill.** The hook JSON (the file shape) lives in
