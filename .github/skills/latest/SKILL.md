...
- **If the working tree is dirty** (staged or unstaged changes — on `main` or any other branch): **stop** unconditionally. Do **not** switch branches *or* pull — both risk losing or entangling uncommitted work. Tell the user to commit first (e.g. via `/ship`), then re-run. There is **no** "confirm to proceed" path — this skill always refuses to act on a dirty working tree.
...
- This skill does not create branches, commit, or push — use `/ship` for that
