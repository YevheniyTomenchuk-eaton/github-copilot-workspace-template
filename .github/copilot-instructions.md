...
This is a **template** for building your own GitHub Copilot-powered workspace — a place where you teach Copilot your conventions once (through instructions, prompts, skills, agents, templates, and scripts) and then let it do repeatable work for you. Clone it, open it in VS Code, and start adding your own content and customizations.
...
**Script it or template it — never inline it.** Prompts, instructions, agents, and skills must stay declarative. Any reusable, multi-step, or error-prone logic belongs in an extracted artifact referenced everywhere — not pasted as plain text:

- **Executable logic → a script.** If a prompt or skill would otherwise embed a shell/PowerShell/Python snippet (an API call sequence, a poll loop, a parse-and-emit step, a validation pass), extract it to `.github/scripts/<domain>/<name>.{ps1,py}` and have the prompt **call** it. Scripts emit machine-readable `KEY=value` lines so callers parse output deterministically. See [`.github/scripts/github/README.md`](scripts/github/README.md) for the established pattern.
- **Repeated file shapes → a template.** If a prompt or instruction would otherwise spell out the full skeleton of a file it tells the AI to create, extract that skeleton to `.github/templates/<path>.template.{md,ext}` and have the instruction **link** to it.
...
- **`.github/prompts/`** — AI prompt files (the `/commands` you run). File naming: `{path-components-joined-by-dots}.{action}.prompt.md`.
...
- **`.github/agents/`** — Custom agent definition files (`{name}.agent.md`) with YAML frontmatter (`name`, `description`, `model`).
- **`.github/skills/`** — Reusable skill packages. Each skill lives in its own subfolder with a `SKILL.md` file. Skills are invoked automatically when the agent detects a matching task.
- **`.github/scripts/`** — Extracted executable logic referenced by prompts, instructions, and skills. Each subfolder matches a domain. Scripts emit machine-readable `KEY=value` lines so callers parse output deterministically, and each domain folder carries a `README.md` cataloguing its scripts.
- **`.github/hooks/`** — Agent hook definition files (`*.json`) that bind a VS Code lifecycle event (`PreToolUse`, `PostToolUse`, …) to a command. Like prompts and templates, each hook uses the dot-path filename, but the file sits **flat** in `.github/hooks/` (e.g. `.github/hooks/workspace.demo.hooks-tour.json` encodes `workspace/demo/`). The command a hook runs is a **script** under `.github/scripts/`, never inlined logic. VS Code's hook loader is **single-level per folder** (no `**` recursion), so flat placement keeps every hook in the default-registered `.github/hooks` folder; only a deliberately nested subfolder needs its own `chat.hookFilesLocations` entry in `.vscode/settings.json`.

**Naming rule:** The dot-prefix in the filename **must encode the directory path** from the subfolder root to the file's parent directory, with `/` replaced by `.`. The filename must never skip, reorder, or abbreviate path segments. If a path component would cause a keyword to repeat (e.g., `template` in the path and as the suffix), omit the path component.

**Leading-dot rule:** A `.` is the dot-prefix **segment separator**, so a path segment whose name begins with a dot (e.g. the `.github` folder itself) is encoded **without its leading dot** — in **both** the dot-prefix filename **and** the mirrored directory path: `.github/prompts/` → `github.prompts` (filename) and `.github/instructions/github/prompts/` (directory), never `.github.prompts` or a nested literal `.github` folder. Example: a file governing `.github/prompts/**` lives at `.github/instructions/github/prompts/github.prompts.instructions.md`.
