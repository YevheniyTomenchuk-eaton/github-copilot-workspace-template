---
description: "Create and register a new toolkit category — README, instructions, prompts folder, gitignore, Jekyll exclude, hub entry"
agent: agent
---
...
5. Read one existing toolkit README as a model:
   ...
   - Script-driven output (prompt writes a spec, a script builds the file) → `toolkit/word/README.md`
...
| Prompts (list of `{action}` + one-liner) | ✅ | `create` → build notes from a changelog |
...
Refuse to proceed if the slug already exists under `toolkit/`, `.github/instructions/toolkit/`, or `.github/prompts/toolkit/`.
...
### 4. Prompts — `.github/prompts/toolkit/{slug}/toolkit.{slug}.{action}.prompt.md`

One file per action collected from the user. Frontmatter:
...
Body sections (in order): `## Task`, `## Context Loading` (numbered file reads — always include `toolkit.instructions.md` and the new category instructions), `## {Action} Steps`, `## User Input` with an example.
...
3. Every prompt file exists at `.github/prompts/toolkit/{slug}/`
...
- **Slug collision.** If a folder with the same slug exists anywhere under `toolkit/`, `.github/instructions/toolkit/`, or `.github/prompts/toolkit/`, stop and ask the user for a different slug.
...
- **Custom agent.** If the user said they need a dedicated agent that doesn't exist, create it under `.github/agents/{name}.agent.md` and use `agent: {name}` in the prompt frontmatter.
...
> *"Create a new toolkit called `release-notes` (📝 Release Notes) that generates release notes from a changelog. One prompt `create` that reads a changelog file and outputs `release-notes.md` into a timestamped folder."*
