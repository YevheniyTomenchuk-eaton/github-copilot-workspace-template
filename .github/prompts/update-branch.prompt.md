...
- This prompt is for **feature branches**, not `main`. To update `main` itself, use `/latest`.
- This prompt does **not** create a PR — it merges, resolves conflicts, commits, and pushes the updated branch. The existing PR updates automatically.
...
- **If on `main`**: stop. This prompt updates a feature branch. Use `/latest` to update `main` instead.
