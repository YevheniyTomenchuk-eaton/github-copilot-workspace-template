...
Run all 5 Python scripts from the repository root. Each script exits 0 on success, non-zero on failure.
...
The 6th CI check (UTF-8 no BOM + CRLF line endings) runs inline in the workflow. [.github/scripts/check-encoding.ps1](../scripts/check-encoding.ps1) reproduces it locally — by default it inspects all changed `.md` files since HEAD:
